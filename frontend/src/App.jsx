...
function SearchBar({ accentColor }) {
  ...
  const handleSelect = (item) => {
    setSelected(item);
    setQuery(item.part_name);
    setOpen(false);
  };

  const handleClear = () => {
    setQuery(""); setResults([]); setOpen(false); setSelected(null);
  };
  ...
  return (
    <div ref={wrapperRef} style={{ position:"relative", width:"100%", maxWidth:480 }}>
      ...
      <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
        <span style={{ position:"absolute", left:12, color:"#64748b", fontSize:16, pointerEvents:"none" }}>
          🔍
        </span>
        <input
          value={query}
      ...
      {open && (
        <div style={{
          ...
          {loading && (
            <div style={{ padding:"14px 16px", color:"#64748b", fontSize:13 }}>Searching...</div>
          )}
          ...
          {!loading && results.map((r, i) => (
            ...
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
                <div style={{ flex:1 }}>
                  {/* Part name with query highlight */}
                  <div style={{ fontWeight:700, color:"#f1f5f9", fontSize:13 }}>
                    {highlightMatch(r.part_name, query)}
                  </div>
          ...
          {!loading && results.length > 0 && (
            <div style={{ padding:"8px 16px", fontSize:11, color:"#475569",
              ...
              {results.length} result{results.length !== 1 ? "s" : ""} — refine your search to narrow down
            </div>
          )}
        </div>
      )}
      ...
      {selected && (
        <div style={{
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div>
              <div style={{ fontWeight:700, color:"#f1f5f9", fontSize:15 }}>{selected.part_name}</div>
              <div style={{ fontSize:12, color:"#64748b", marginTop:4 }}>
                {selected.part_number} · {selected.supplier}
              </div>
              <div style={{ fontSize:12, color:"#94a3b8", marginTop:6 }}>
                <span style={{ color: brandColors[selected.car_brand] || "#60a5fa", fontWeight:600 }}>
                  {selected.car_brand}
                </span>
                {" "}{selected.car_model} · {selected.variant} · {selected.year} · {selected.engine}
              </div>
            </div>
            <div style={{ textAlign:"right" }}>
              ...
              <div style={{
                ...
                {selected.stock>10?"In Stock":selected.stock>3?`Low stock (${selected.stock})`:`Last ${selected.stock}`}
              </div>
            </div>
          </div>
          <button onClick={handleClear} style={{
...
function highlightMatch(text, query) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <span style={{ background:"#1d4ed8", color:"#fff", borderRadius:3, padding:"0 2px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#0f172a 0%,#1e1b4b 100%)",
        ...
        <div style={{ flexShrink:0 }}>
          ...
          <div style={{ fontSize:10, color:"#64748b", letterSpacing:2, textTransform:"uppercase" }}>
            {brand ? `${brand.name} Brake Parts` : "Multi-Brand Brake Parts"}
          </div>
        </div>
