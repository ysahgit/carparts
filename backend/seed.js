...
async function seed() {
  ...
  // ── Brand / Model / Variant definitions ─────────────────────────────────────
  ...
  const brandData = [
    ...
    },
    {
      ...
      models: [
        ...
        { id: 6,  name: 'A4',  variants: [['35 TFSI','1.5L 4-cyl Turbo'],['40 TFSI','2.0L 4-cyl Turbo'],['45 TFSI','2.0L 4-cyl Turbo 265hp'],['S4','3.0L V6 Turbo']] },
  ...
  // ── Per-brand part templates ─────────────────────────────────────────────────

  // Each brand has a function: (variantName, slug) => array of part rows
  // [name, part_number, price, stock, partsBrand, notes]

  const bmwPadPrices = {
    '318i':19,'320i':22,'330i':28,'M340i':68,
    '520i':24,'530i':30,'540i':44,'M550i':95,
    'xDrive20i':26,'xDrive30i':32,'M40i':72,
    '116i':17,'118i':17,'120i':21,'M135i':62,
  };
  const audiPadPrices = {
    '30 TFSI':18,'35 TFSI':22,'40 TFSI':27,'S3':58,
    '35 TFSI':21,'45 TFSI':30,'S4':75,
    '40 TDI':25,'55 TFSI':48,'SQ5':82,
    'A6 40 TDI':28,'A6 45 TFSI':36,'S6':88,
  };
  const vwPadPrices = {
    '1.0 TSI':15,'1.5 TSI':18,'2.0 TDI':20,'GTI':45,
    'Passat 1.5 TSI':19,'2.0 TSI':24,'GTE':28,
    'Tiguan 2.0 TDI':22,'2.0 TSI 4Motion':28,'R':55,
    '1.0 MPI':14,'Polo GTI':42,
  };
  const toyotaPadPrices = {
    '1.2T':16,'1.8 Hybrid':20,'2.0 GR Sport':26,'GR Corolla':55,
    '2.0 AWD-i':22,'2.5 Hybrid':24,'2.5 PHEV':28,'2.5 AWD':24,
    '1.0 VVT-i':13,'1.5 VVT-i':16,'1.5 Hybrid':18,'GR Yaris':52,
    '1.8 C-HR Hybrid':19,'2.0 C-HR Hybrid':22,'2.0 GR Sport Hybrid':30,
  };

  const r = (min=2, range=15) => Math.ceil(Math.random() * range + min);

  const bmwParts = (vname, slug) => {
    const fp = bmwPadPrices[vname] || 25;
    const fpn = vname.startsWith('M') ? `Brembo P060${r(80,19)}` : `Bosch BP${1800+r(0,299)}`;
    return [
  ...
  const audiParts = (vname, slug) => {
    const fp = audiPadPrices[vname] || 27;
    const isS = vname.startsWith('S') || vname.startsWith('RS');
    const fpn = isS ? `Brembo P85${r(0,99).toString().padStart(3,'0')}` : `TRW GDB${1700+r(0,199)}`;
    return [
  ...
  const vwParts = (vname, slug) => {
    const fp = vwPadPrices[vname] || 18;
  ...
  const toyotaParts = (vname, slug) => {
    const fp = toyotaPadPrices[vname] || 20;
    const isGR = vname.startsWith('GR');
    const fpn = isGR ? `Endless MX72 ${slug}` : `Nisshinbo NP${1000+r(0,999)}`;
    return [
      ...
      ...(vname.includes('Hybrid') ? [
        ['Brake Booster (Hybrid)',  `TOY-BB-${slug}`,  fp*8,    r(1,3),'Toyota Genuine', 'Hybrid e-booster'],
      ] : []),
    ];
  };

  const partFns = { BMW: bmwParts, Audi: audiParts, Volkswagen: vwParts, Toyota: toyotaParts };
    ...
    for (const model of brand.models) {
      ...
      for (let year = 2018; year <= 2023; year++) {
        ...
        for (const [vname, engine] of model.variants) {
          ...
          const parts = partFns[brand.name](vname, slug);
  ...
  fs.writeFileSync('bmw_parts.db', Buffer.from(data));
  console.log(`Seeded OK: ${pid-1} parts | ${vid-1} variants | ${yid-1} year rows`);
  db.close();
}
